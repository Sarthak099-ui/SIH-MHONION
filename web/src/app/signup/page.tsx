...
export default function SignupPage() {
  ...
  const performSignupRedirect = (selectedRole: UserRole, name: string) => {
    document.cookie = `sih_demo_role=${selectedRole}; path=/; max-age=86400`;
    ...
    else if (selectedRole === 'grader') router.push('/grader/dashboard');
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const cleanName = fullName.trim() || 'Valued User';
    ...
    if (isSupabaseConfigured()) {
      try {
        ...
        if (!error && data.user) {
          ...
          performSignupRedirect(role, cleanName);
          setLoading(false);
          return;
        }
      } catch {
        // Fallback to local profile registration
      }
    }

    // 2. Intelligent Local / Demo Registration
    try {
      let registeredUsers: Record<string, { fullName: string; role: UserRole }> = {};
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-200">
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <button
                  ...
                  className={`p-3 rounded-xl border text-center flex flex-col items-center gap-1 transition-all cursor-pointer ${role === 'farmer' ? 'border-emerald-600 bg-emerald-50/80 text-emerald-900 ring-2 ring-emerald-500/20 font-bold' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}
                >
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border text-center flex flex-col items-center gap-1 transition-all cursor-pointer ${role === 'grader' ? 'border-amber-600 bg-amber-50/80 text-amber-900 ring-2 ring-amber-500/20 font-bold' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}
                >
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border text-center flex flex-col items-center gap-1 transition-all cursor-pointer ${role === 'buyer' ? 'border-blue-600 bg-blue-50/80 text-blue-900 ring-2 ring-blue-500/20 font-bold' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}
                >
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Full Name / Organization</label>
            ...
            >
              {loading ? 'Creating Account...' : `Register & Open ${role.charAt(0).toUpperCase() + role.slice(1)} Portal`}
